...
public:
    BrowserHistory(std::string homepage) {
        ...
        currItr = history.begin();
    }
    
    void visit(std::string url) {
        std::list<std::string>::iterator delStartItr = currItr;
        delStartItr++;
        history.erase(delStartItr, history.end() );
        history.push_back(url);
        currItr++;
    }
    
    std::string back(int steps) {
        while (steps-- > 0 && currItr != history.begin() ) {
            currItr--;
        }
        return *currItr;
    }
    ...
    std::string forward(int steps) {
        while (steps-- > 0 && currItr != (--history.end()) ) {
            currItr++;
        }
        return *currItr;
    }
...
private:
    ...
    std::list<std::string>::iterator currItr;
};
...
// n = history length
// k = operation steps
// Time: visit: O(k), back: O(k), forward: O(k)
// Space: O(n)