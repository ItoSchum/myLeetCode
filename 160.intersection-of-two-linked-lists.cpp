...
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        // Init reference map
        std::unordered_set<ListNode* > allNodeInB;
        ListNode* currNode = headB;
        while (currNode != nullptr) {
            allNodeInB.insert(currNode);
            currNode = currNode->next;
        }
        // Search in list A
        currNode = headA;
        while(currNode != nullptr) {
            if ( allNodeInB.find(currNode) != allNodeInB.end() ) {
                return currNode;
            }
            currNode = currNode->next;
        }
...
// n = node number of A, m = node number of B
// Time: O(n + m)
// Space: O(m)
