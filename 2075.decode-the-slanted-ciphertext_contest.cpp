    ...
    std::string decodeCiphertext(std::string encodedText, int rows) {
        ...
        for (int startCol = 0; startCol < cols; startCol++) {
            for (int row = 0, col = startCol; row < rows && col < cols; ++row, ++col) {
                // std::cout << row <<  ", " << col << " = " << coordToIdx(row, col, cols) << std::endl;
                originalStr += encodedText[ coordToIdx(row, col, cols) ];
            }
        }
        // trim trailing space
        std::size_t end = originalStr.find_last_not_of(' ');
        originalStr = originalStr.substr(0, end + 1);
        
        return originalStr;
    }
    
    int coordToIdx(int row, int col, int cols) {
        return row * cols + col;
    }
};