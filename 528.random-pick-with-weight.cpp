...
public:
    Solution(vector<int>& w) {
        accumulatingVec.push_back(w[0]);
        if (w.size() == 1) {
            return;
        }
        for (int i = 1; i < w.size(); ++i) {
            accumulatingVec.push_back(accumulatingVec[i - 1] + w[i]);
        }
    }
    
    int pickIndex() {
        if (accumulatingVec.size() == 1){
            return 0;
        }
        int randVal = rand() % accumulatingVec.back();
        std::vector<int>::iterator randValItr = std::upper_bound(accumulatingVec.begin(), accumulatingVec.end(), randVal);
        return randValItr - accumulatingVec.begin();
    }

private:
    std::vector<int> accumulatingVec;
};
